...
until mysqladmin ping -h "${DB_HOST}" -u "${DB_USER}" ${DB_PASS:+-p"${DB_PASS}"} --silent 2>/dev/null; do
    sleep 2
done
echo "MySQL is up."

# Initialize schema if the user table doesn't exist yet
TABLE_EXISTS=$(mysql -h "${DB_HOST}" -u "${DB_USER}" ${DB_PASS:+-p"${DB_PASS}"} \
...
if [ "${TABLE_EXISTS}" = "0" ]; then
    ...
    mysql -h "${DB_HOST}" -u "${DB_USER}" ${DB_PASS:+-p"${DB_PASS}"} "${DB_NAME}" \
